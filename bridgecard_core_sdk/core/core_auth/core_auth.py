...
from bridgecard_core_sdk.core.core_auth.error import InvalidToken,AuthenticationTokenMismatch, IssuingPermissionHasBeenDeactivated
...
async def verify_token(token: Optional[str] = Header(None), request: Request = None):
    ...
    data = core_db_usecase.admin_repository.filter_db(filter_string, token)
    ...
    if client_details["is_account_verified"] == False and client_details["environment"] == EnvironmentEnum.production:
...
def get_client_from_issuing_id(issuing_app_id: str, environment: EnvironmentEnum):
    
    request_time = int(time.time())
    data = core_db_usecase.admin_repository.filter_db("issuing_app_id", issuing_app_id)
